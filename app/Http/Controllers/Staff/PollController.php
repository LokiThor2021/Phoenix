...
class PollController extends Controller
{
    ...
    public function store(StorePoll $storePoll): \Illuminate\Http\RedirectResponse
    {
        ...
        $options = \collect($storePoll->input('options'))->map(fn ($value): \App\Models\Option => new Option(['name' => $value]));
    ...
    public function update(StorePoll $storePoll, int $id): \Illuminate\Http\RedirectResponse
    {
        ...
        $existingOldOptionIds = \collect($storePoll->input('option-id'))->map(fn ($id): int => (int) $id)->all();
        ...
        $existingOldOptionContents = \collect($storePoll->input('option-content'))->map(fn ($content): string => (string) $content)->all();
        ...
        // Insert new options
        $newOptions = \collect($storePoll->input('new-option-content'))->map(fn ($content): \App\Models\Option => new Option(['name' => $content]));
